// svg-icon
// import LineMd from '@iconify-json/line-md';
let ICONIFY_API = 'https://icon.jiayupearl.shop'
const iconifyPresets = {
  // LineMd
}
const customerSvg = {
...
function presetsIconify(id) {
  ...
  if (parts.length === 1) {
    ...
    if (parts.length > 1) {
      // prefix-icon.svg
      prefix = svgNameSwtich(parts.shift())
      icon = parts.join('-')
    }
  }
...
export function getLocalSVG(id) {
...
export function iconifySetup(api) {
  if (api) {
    ICONIFY_API = api
